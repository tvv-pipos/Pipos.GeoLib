...
{
    public async static Task<(List<Node>, List<Edge>)> ReadDataGotland(string connectionString)
    {
        var sql = @"
    ...
    public async static Task<(List<Node>, List<Edge>)> ReadData(string connectionString)
    {
        var sql = @"
            SELECT ST_AsText(the_geom) AS WKT, id, b_kkod, f_kkod, function_class, network_group 
            FROM road_segment";
    ...
    private async static Task<(List<Node>, List<Edge>)> ReadData(string connectionString, string sql)
    {
        ...
        await using (var reader = await cmd.ExecuteReaderAsync())
        {
            while (await reader.ReadAsync())
            {
                ...
                var (nodes, edges) = Parser.ParseNodes(lineString, forwardSpeed, backwardSpeed, networkGroup, functionClass);
                if (nodes.Count > 1) 
                {
        ...
        return (result.Values.ToList(), allEdges);
    }
