...
public static class Network
{
    public static async Task<RoutingGraph> LoadFullNVDB(Scenario scenario)
    {
        ...
        var connectionString = "Server=localhost;database=pipos;user id=REMOVED_SECRET;password=REMOVED_SECRET;port=5432";
    ...
    public static async Task<RoutingGraph> LoadOptimizedNVDB(Scenario scenario)
    {
        var connectionString = "Server=localhost;database=pipos;user id=REMOVED_SECRET;password=REMOVED_SECRET;port=5432";
        var connectionStringrut = "Server=localhost;database=pipos;user id=REMOVED_SECRET;password=REMOVED_SECRET;port=5432";


        List<Node> nodes = await NVDB.ReadData(connectionString, scenario);

        var activity_tiles = await ActivityTile.ReadPopulationTilesFromDb(connectionStringrut, scenario);
        activity_tiles.AddRange(await TravelReason.ReadTravelReasonTiles(connectionStringrut, scenario));
